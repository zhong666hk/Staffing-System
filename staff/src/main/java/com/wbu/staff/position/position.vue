...
export default defineComponent({
  ...
  setup() {
    ...
    const onDelete = (record) => {
      axios.delete("/position/admin/position/delete/" + record.id).then((response) => {
    ...
    const handleOk = () => {
      axios.post("/position/admin/position/save", position.value).then((response) => {
        if (response.code===200) {
          notification.success({description: response.message});
          visible.value = false;
          handleQuery({
    ...
    const handleQuery = (param) => {
      ...
      loading.value = true;
      axios.get("/position/admin/position/query-list", {
        params: {
          page: param.page,
          size: param.size
        }
