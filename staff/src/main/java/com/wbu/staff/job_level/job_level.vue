...
export default defineComponent({
  ...
  setup() {
    ...
    const onDelete = (record) => {
      axios.delete("/jobLevel/admin/jobLevel/delete/" + record.id).then((response) => {
    ...
    const handleOk = () => {
      axios.post("/jobLevel/admin/jobLevel/save", jobLevel.value).then((response) => {
        if (response.code===200) {
          notification.success({description: response.message});
          visible.value = false;
          handleQuery({
    ...
    const handleQuery = (param) => {
      ...
      loading.value = true;
      axios.get("/jobLevel/admin/jobLevel/query-list", {
        params: {
          page: param.page,
          size: param.size
        }
